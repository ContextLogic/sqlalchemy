...
# PATCH(chutong): Change the format of displaying string literal to fit TD.
BIND_TEMPLATES = {
    'pyformat':"%%(%(name)s)s",
    'qmark':"?",
    'format':"%%s",
...
class SQLCompiler(engine.Compiled):
    ...
    def visit_column(self, column, result_map=None, **kwargs):
        # PATCH(chutong): Named column in format of v['%s'] to fit td
        # query purpose. If we are using a label from a table alias,
        ...
        if table is None or not table.named_with_column:
            ...
            # PATCH(chutong): Removed table name quote to fit td.
            return schema_prefix + tablename + \
                    "." + name
    ...
    def visit_cast(self, cast, **kwargs):
        totype = cast.typeclause._compiler_dispatch(self, **kwargs)

        # PATCH(chutong): Apparently, TD only accepts INT instead of INTEGER,
        # add this to make cast work for TD.
        if totype == 'INTEGER':
            totype = 'INT'

        return "CAST(%s AS %s)" % \
                    (cast.clause._compiler_dispatch(self, **kwargs), totype)
    ...
    def visit_bindparam(self, bindparam, within_columns_clause=False,
        ...
        # PATCH(chutong): Expanded literal params into the query for td to use.
        return self.bindparam_string(bindparam.value)
    ...
    def bindparam_string(self, name):
        # PATCH(chutong): Display column name to fit TD.
        # If name is numeric, display in format of number such as: 123,
        # otherwise, display it in format of string such as: 'abc'.
    ...
    def visit_alias(self, alias, asfrom=False, ashint=False, 
        ...
        if ashint:
            ...
            # PATCH(chutong): Removed 'AS' keyword for table aliasing
            # to adhere to TD query standard.
    ...
    def visit_select(self, select, asfrom=False, parens=True, 
        ...
        # PATCH(chutong): Removed newline character to generate query on one
        # line.
        if froms:
            text += " FROM "
    ...
    def limit_clause(self, select):
        ...
        if select._limit is not None:
            text +=  " LIMIT " + self.process(sql.literal(select._limit))
    ...
    def visit_join(self, join, asfrom=False, **kwargs):
        # PATCH(chutong): Added isfullouter to support full outer join.
        return (
            join.left._compiler_dispatch(self, asfrom=True, **kwargs) + 
            (join.isfullouter and " FULL OUTER JOIN " or
...
class IdentifierPreparer(object):
    ...
    def _requires_quotes(self, value):
        ...
        # PATCH(chutong): As we are referring to a column or table in TD, do not
        # do any string trim.
        return False
